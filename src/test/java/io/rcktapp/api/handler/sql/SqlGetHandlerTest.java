...
public class SqlGetHandlerTest {
    ...
    public void testSqlGetHandler_queryParametersAreProperlyPassedToQueryObjectsMethod() throws Exception {
        ...
        try (MockedStatic<Rql> rqlMockedStatic = mockStatic(Rql.class)) {
            try (MockedStatic<Sql> sqlMockedStatic = mockStatic(Sql.class)) {
    ...
    @Test
    public void testSqlGetHandler_queryParametersAreProperlyPassedToQueryObjectsMethodWithRqlParameters() throws Exception {

        Url url = new Url("http://localhost:8080/api/lift/us/plugins/updatexml(rand(),concat(CHAR(126),user(),CHAR(126)),null)?name1=asdf&name2=qwer");
        ...
        try (MockedStatic<Rql> rqlMockedStatic = mockStatic(Rql.class)) {
            try (MockedStatic<Sql> sqlMockedStatic = mockStatic(Sql.class)) {
                ...
                // Verify that the passed in query contains no SQL functions
                verify(sqlRql).createStmt(sqlCaptor.capture(), any(), rqlParamsCaptor.capture(), any(Replacer.class));
                assertEquals(expectedQuery, sqlCaptor.getValue());
                // Assert that we are passing in the correct rql parameters parsed from the URL
                assertEquals(request.getParams(), rqlParamsCaptor.getValue());
