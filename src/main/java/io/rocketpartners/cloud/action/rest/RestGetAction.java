...
public class RestGetAction extends Action<RestGetAction> {
  ...
  /**
   * These params are specifically NOT passed to the Query for parsing. These are either dirty
   * worlds like sql injection tokens or the are used by actions themselves
   */
  protected Set reservedParams =
      new HashSet(
          Arrays.asList(
              "select",
              "insert",
  ...
  protected static String getEntityKey(Object obj) {
    ...
    if (obj instanceof ObjectNode) obj = ((ObjectNode) obj).get("href");

    String str = (String) obj;
    int idx = str.lastIndexOf('/');
    if (idx > 0) str = str.substring(idx + 1, str.length());
    return str;
  }
  ...
  static void mapToAttributes(Collection collection, Term term) {
    ...
    if (term.isLeaf() && !term.isQuoted()) {
      ...
      if (attrName != null) term.withToken(attrName);
    } else {
  ...
  protected static String stripTerm(String str, String startToken, char... endingTokens) {
    ...
    for (char c : endingTokens) tokens.add(c);

    while (true) {
      int start = str.toLowerCase().indexOf(startToken);

      // this makes sure the char before the start token is not a letter or number which would mean
      // we are in the
      // middle of another token, not at the start of a token.
      while (start > 0
          && (Character.isAlphabetic(str.charAt(start - 1)) || Character.isDigit(start - 1)))
      ...
      if (start > -1) {
        ...
        while (end < str.length()) {
          ...
          if (tokens.contains(c)) break;
          end += 1;
        }

        if (end == str.length()) str = beginning;
        else str = beginning + str.substring(end + 1, str.length());
      } else {
        break;
      }
    }
  ...
  protected static String expandPath(String path, Object next) {
    if (Utils.empty(path)) return next + "";
    else return path + "." + next;
  }
  ...
  protected static boolean shouldExpand(Set<String> expands, String path, Relationship rel) {
    ...
    for (String ep : expands) {
      if (ep.startsWith(path)
          && (ep.length() == path.length() || ep.charAt(path.length()) == '.')) {
        expand = true;
        break;
      }
    }
  ...
  protected static boolean exclude(String path, Set<String> includes, Set<String> excludes) {
    ...
    if (includes.size() > 0 || excludes.size() > 0) {
      ...
      if (includes != null && includes.size() > 0) {
        if (!find(includes, path, true)) exclude = true;
      }

      if (excludes != null && excludes.size() > 0) {
        if (find(excludes, path, false)) exclude = true;
      }
    }

    // System.out.println("exclude(" + path + ", " + includes + ", " + excludes + ") -> " +
    // exclude);
  ...
  protected static boolean find(
      java.util.Collection<String> params, String path, boolean matchStart) {
    boolean rtval = false;
    ...
    if (params.contains(path)) {
      ...
      for (String param : params) {
        ...
        if (Utils.wildcardMatch(param, path)) rtval = true;
      }
    }
    ...
    if (req.getSubCollectionKey() != null) {
      ...
      if (rel == null)
        throw new ApiException(
            SC.SC_404_NOT_FOUND, "'" + req.getSubCollectionKey() + "' is not a valid relationship");
      ...
      if (rel.isManyToOne()) {
        ...
        for (Column fk : fkIdx.getColumns()) {
          ...
          if (pkVal == null)
            throw new ApiException(
                SC.SC_400_BAD_REQUEST, "Missing parameter for foreign key column '" + fk + "'");
        ...
        List<KeyValue> rows =
            getRelatedKeys(rel.getFkIndex1(), rel.getFkIndex2(), Arrays.asList(entityKey));
        if (rows.size() > 0) {
        ...
        // The link was requested like this  :
        // http://localhost/northwind/source/orderdetails/XXXXX/order
      ...
      Term term =
          Term.term(
              null,
              "_key",
              req.getCollection().getEntity().getTable().getPrimaryIndex().getName(),
              entityKeys.toArray());
  ...
  protected Results<ObjectNode> select(
      Request req, Collection collection, Map<String, String> params, Api api) throws Exception {
    // ------------------------------------------------
    ...
    if (params.size() > 0) {
      ...
      for (String paramName : params.keySet()) {
        ...
        if (term.hasToken("eq") && reservedParams.contains(term.getToken(0))) continue;
        ...
        if (term.hasToken("eq") && term.getTerm(0).hasToken("includes")) {
          ...
          // dotted paths, don't pass on to the Query the extra stuff will be removed by the rest
          // action.
          ...
          for (Term child : term.getTerms()) {
            if (child.hasToken("href") && collection != null) {
              ...
              for (Column c : pk.getColumns()) {
                ...
                if (!includesPkCol) term.withTerm(Term.term(term, c.getName()));
              }
              break;
            }
          }
    ...
    if (collection == null) {
      ...
      if (db == null)
        throw new ApiException(
            SC.SC_400_BAD_REQUEST, "Unable to find collection for url '" + req.getUrl() + "'");
    ...
    if (results.size() > 0) {
      ...
      for (int i = 0; i < results.size(); i++) {
        ...
        if (collection == null) {
          ...
          if (Utils.empty(entityKey))
            throw new ApiException(
                SC.SC_500_INTERNAL_SERVER_ERROR, "Unable to determine entity key for " + row);
      ...
      if (collection != null) expand(req, collection, results.getRows(), null, null, null);
  ...
  /**
   * This is more complicated that it seems like it would need to be because it attempts to retrieve
   * all values of a relationship at a time for the whole document. It does not run a recursive
   * query for each entity and each relationship which could mean hundreds and hundreds of queries
   * per document. This should result in number of queries proportional to the number of expands
   * terms that does not increase with the number of results at any level of the expansion.
   */
  protected void expand(
      Request request,
      Collection collection,
      List<ObjectNode> parentObjs,
      Set expands,
      String expandsPath,
      MultiKeyMap pkCache)
      throws Exception {
    if (parentObjs.size() == 0) return;

    if (expands == null) expands = Chain.peek().mergeEndpointActionParamsConfig("expands");

    if (expandsPath == null) expandsPath = "";

    for (Relationship rel : collection.getEntity().getRelationships()) {
      boolean shouldExpand = shouldExpand(expands, expandsPath, rel);

      // System.out.println("should expand " + Chain.getDepth() + " -> " + rel + " -> " +
      // shouldExpand);
      ...
      if (shouldExpand) {
        if (pkCache == null) {
          ...
          pkCache =
              new MultiKeyMap() {
                ...
                //               {
                //                  System.out.println("PUTPUTPUTPUTPUTPUTPUTPUT:  " + key1 + ", " +
                // key2);
                ...
                //                  System.out.println("GETGETGETGETGETGETGETGET: " + key1 + ", " +
                // key2 + " -> " + value);
                //                  return value;
                //               }
              };
        ...
        if (rel.isOneToMany()) {
          ...
          //               idxToRetrieve =
          // rel.getRelated().getTable().getPrimaryIndex();//Entity().getKey().getColumn();
            ...
            if (!toMatchEks.contains(parentEk)) {
              if (parentObj.get(rel.getName()) instanceof ArrayNode)
                throw new ApiException(
                    ...
                    "Algorithm implementation error...this relationship seems to have already been expanded.");
        ...
        for (KeyValue<String, String> row : relatedEks) {
          // the values in the many_to_many link table may have different names than the target
          // columns so you have to
        ...
        List<ObjectNode> newChildObjs =
            recursiveGet(
                pkCache,
                relatedCollection,
                unfetchedChildEks,
                expandPath(expandsPath, rel.getName()));
  ...
  protected List<KeyValue> getRelatedKeys(
      Index idxToMatch, Index idxToRetrieve, List<String> toMatchEks) throws Exception {
    if (idxToMatch.getTable() != idxToRetrieve.getTable())
      throw new ApiException(
          SC.SC_400_BAD_REQUEST, "You can only retrieve corolated index keys from the same table.");
    ...
    Rows rows =
        ((Rows)
            idxToMatch
                ...
                .getTable()
                .getDb()
                .select(idxToRetrieve.getTable(), terms)
                .getRows());
    for (Row row : rows) {
      List keyParts = row.asList();
      String parentEk = Table.encodeKey(keyParts.subList(0, idxToMatch.getColumns().size()));
      String relatedEk =
          Table.encodeKey(keyParts.subList(idxToMatch.getColumns().size(), keyParts.size()));
  ...
  protected List<ObjectNode> recursiveGet(
      MultiKeyMap pkCache,
      Collection collection,
      java.util.Collection entityKeys,
      String expandsPath)
      throws Exception {
    if (entityKeys.size() == 0) return Collections.EMPTY_LIST;
    ...
    for (String key : params.keySet()) {
      ...
      if (reservedParams.contains(lcKey)) continue;
      ...
      if (lcKey.matches(".*\\b" + lcPath.replace(".", "\\.") + ".*")) {
        ...
        if (!Utils.empty(value)) url += "=" + URLEncoder.encode(value, "UTF-8");
      }
    }
    ...
    if (sc == 404) {
      ...
      else throw new ApiException(SC.SC_500_INTERNAL_SERVER_ERROR, res.getText());
    } else if (sc == 200) {
      ...
      for (ObjectNode node : nodes) {
        ...
        if (pkCache.containsKey(collection, entityKey))
          throw new ApiException(
              SC.SC_500_INTERNAL_SERVER_ERROR, "FIX ME IF FOUND.  Algorithm Implementation Error");
    ...
    throw new ApiException(
        SC.SC_500_INTERNAL_SERVER_ERROR,
        "Unknow repose code \"" + sc + "\" or body type from nested query.");
  }
  ...
  //      }
  //
  //      System.out.println("include(" + path + ", " + includes + ", " + excludes + ") -> " +
  // include);
  //
  //      return include;
  //   }
