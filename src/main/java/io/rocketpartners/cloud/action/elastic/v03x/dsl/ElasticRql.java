...
public class ElasticRql extends Rql {
  ...
  public QueryDsl toQueryDsl(Map<String, String> params) throws Exception {
    ...
    String sizeStr =
        caseInsensitiveParams.get("pagesize"); // if no pagesize is specified, assume max_limit

    int size = stmt.getMaxRows();
    if (sizeStr != null) size = toInt("pagesize", sizeStr);
    String pageStr = caseInsensitiveParams.get("pagenum");
    int pageNum = 1;
    if (pageStr != null) pageNum = toInt("pageNum", pageStr);
    ...
    if (caseInsensitiveParams.containsKey("start")) {
      List<String> searchAfterList =
          Arrays.asList(caseInsensitiveParams.remove("start").split(","));
      if (pageNum * size > MAX_NORMAL_ELASTIC_QUERY_SIZE - 1) {
        for (int i = 0; i < searchAfterList.size(); i++) {
          if (searchAfterList
              .get(i)
              .equals(
                  "[NULL]")) // [NULL] is used to indicate an actual null value, not a null string.
          searchAfterList.set(i, null);
        }
        query.setSearchAfter(searchAfterList);
      }

      // use this value if wantedpage was not set; prevents having to lookup the prev value...of
      // course.
    ...
    for (io.rocketpartners.cloud.action.elastic.v03x.rql.Order order : orderList) {
      if (Parser.dequote(order.col).equalsIgnoreCase("id")) idSortExists = true;
    }
    if (!idSortExists)
    ...
    for (int i = 0; i < orderList.size(); i++) {
      ...
      if (elasticOrder == null) elasticOrder = new Order(Parser.dequote(order.col), order.dir);
      else elasticOrder.addOrder(Parser.dequote(order.col), order.dir);
    }
    ...
    if (elastic != null) {
      if (elastic instanceof Range) {
        ...
        if (((Term) elastic).getToken().equals("ne")) {
          ...
          dsl.setBool(bool);
        } else dsl.setTerm((Term) elastic);
      } else if (elastic instanceof BoolQuery) {
        dsl.setBool((BoolQuery) elastic);
      } else if (elastic instanceof NestedQuery) dsl.setNested((NestedQuery) elastic);
      else if (elastic instanceof FuzzyQuery) dsl.setFuzzy((FuzzyQuery) elastic);
    }
  }
  ...
  private ElasticQuery convertPredicate(Predicate pred) throws Exception {
    ...
    if (pred.terms.size() > 0) {
      for (int i = 0; i < pred.terms.size(); i++) {
        ...
        if (elastic != null) elasticList.add(elastic);
      }
    ...
    switch (pred.token) {
      ...
      case "eq": // equal
        if (pred.terms.get(1).token.contains("*")) {
          ...
          elastic =
              new Term(
                  pred.terms.get(0).token, Parser.dequote(pred.terms.get(1).token), pred.token);
        }
        break;
      case "ne": // not equal
        elastic =
            new Term(pred.terms.get(0).token, Parser.dequote(pred.terms.get(1).token), pred.token);
        break;
      case "and":
      ...
      case "or":
        ...
        for (ElasticQuery eq : elasticList) ((BoolQuery) elastic).addShould(eq);
        break;
      case "sw":
      ...
      case "wo":
        ...
        ((BoolQuery) elastic)
            .addMustNot(
                new Wildcard(
                    pred.terms.get(0).token, "*" + Parser.dequote(pred.terms.get(1).token) + "*"));
        break;
      ...
      case "nemp": // checks for empty strings AND null values
        elastic = new BoolQuery();
        mustNotBool =
            new BoolQuery(); // 'mustNotBool' is used even-though it should be named 'mustBool'
      ...
      case "in":
        ...
        for (Predicate pTerm : termsList) ((Term) elastic).addValue(Parser.dequote(pTerm.token));
        break;
      case "out":
        elastic = new BoolQuery();
        termsList = new ArrayList<Predicate>(pred.terms);
        Term term = new Term(termsList.remove(0).token, pred.token);
        for (Predicate pTerm : termsList) term.addValue(Parser.dequote(pTerm.token));
  ...
  private BoolQuery withWildCardPopulater(Predicate pred, WithType withType) {
    ...
    for (int i = 1; i < pred.terms.size(); i++) {
      switch (withType) {
        case WITH:
          bq.addShould(
              new Wildcard(termToken, "*" + Parser.dequote(pred.terms.get(i).token) + "*"));
          break;
        case STARTS_WITH:
