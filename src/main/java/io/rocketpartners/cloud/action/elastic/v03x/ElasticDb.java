...
public class ElasticDb extends Db<ElasticDb> {
  ...
  private void reflectDb() throws Exception {
    ...
    if (allResp.isSuccess()) {
      // we now have the indices, aliases for each index, and mappings (and settings if we need
      // them)
      ...
      // a map is needed when building tables to keep track of which alias'ed indexes, such as
      // 'all', have previously been built.
      ...
      throw new ApiException(
          SC.matches(allResp.getStatusCode(), allowedFailResponseCodes)
              ? SC.SC_MAP.get(allResp.getStatus())
  ...
  /**
   * At the time of writing, there is no need to parse settings. This method creates tables based on
   * alias names of elastic indexes. If no alias exists, no table is created.
   *
   * <p>The name of the elastic index will be used as a table index. Most tables will only have one
   * index. An example of a table with multiple indexes would be the alias 'all'.
   *
   * @param indexName
   * @param jsIndex
   * @return
  ...
  private void buildAliasTables(
      String elasticName, ObjectNode jsIndex, Map<String, Table> tableMap) {
    ...
    if (jsMappingsDocPropsNode != null) {
      ...
      for (Map.Entry<String, ObjectNode> propEntry : jsAliasProps.entrySet()) {
        ...
        if (tableMap.containsKey(aliasName)) table = tableMap.get(aliasName);
        else {
  ...
  private void addColumns(
      Table table, boolean nullable, Map<String, ObjectNode> jsPropsMap, String parentPrefix) {
