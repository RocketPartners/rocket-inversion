...
public class Utils {
  ...
  private static final Pattern encodeJsonPattern =
      Pattern.compile("[\\p{Cntrl}\\p{Cc}\\p{Cf}\\p{Co}\\p{Cn}\u00A0&&[^\r\n\t]]");
  ...
  static void writeNode(
      ObjectNode node, JsonGenerator json, HashSet visited, boolean lowercaseNames)
      throws Exception {
    ...
    if (href != null) json.writeStringField("href", href.getValue() + "");
    ...
    for (String key : node.keySet()) {
      ...
      if (value == null) {
        ...
        if (!lowercaseNames) json.writeFieldName(name);
        else json.writeFieldName(name.toLowerCase());
  ...
  static void writeArrayNode(
      ArrayNode array, JsonGenerator json, HashSet visited, boolean lowercaseNames)
      throws Exception {
   ...
   * @param obj1
   * @param obj2
   * @return Test for strict == equality, then .equals() equality, then .toString().equals()
   *     equality. Either param can be null.
   */
  ...
  public static boolean empty(Object... arr) {
    ...
    for (int i = 0; empty && arr != null && i < arr.length; i++) {
      Object obj = arr[i];
      if (obj != null && obj.toString().length() > 0) empty = false;
    }
    return empty;
  }
  ...
  public static String implode(String glue, Object... pieces) {
    ...
    for (int i = 0; pieces != null && i < pieces.length; i++) {
      if (pieces[i] != null) {
        String piece =
            pieces[i] instanceof Collection ? implode(glue, pieces[i]) : pieces[i].toString();
   ...
   * @param delim
   * @param pieces
   * @return Same as String.split() but performs a trim() on each piece and returns an list instead
   *     of an array
   */
  ...
  public static List deconstructed(List found, Object... terms) {
    if (terms.length == 1 && terms[0].getClass().isArray()) terms = (Object[]) terms[0];
  ...
  public static String dequote(String str, char[] quoteChars) {
    ...
    while (str.length() >= 2
        && str.charAt(0)
            == str.charAt(
                str.length()
                    - 1)) // && (str.charAt(0) == '\'' || str.charAt(0) == '"' || str.charAt(0) ==
                          // '`'))
    {
  ...
  /**
   * Attempts an ISO8601 data as
   * yyyy-MM-dd|yyyyMMdd][T(hh:mm[:ss[.sss]]|hhmm[ss[.sss]])]?[Z|[+-]hh[:]mm], then yyyy-MM-dd, then
   * MM/dd/yy, then MM/dd/yyyy, then yyyyMMdd
   *
   * @param date
   * @return
   */
  ...
  public static Response assertDebug(Response resp, String lineMatch, String... matches) {
    ...
    if (matches == null || matches.length == 0) return resp;
  ...
  public static List<String> split(String string, char splitOn, char... quoteChars) {
    ...
    for (char c : quoteChars) quotes.add(c);
    ...
    if (buff.length() > 0) strings.add(buff.toString());

    return strings;
  }

  public static void assertEq(Object expected, Object found) {
    if (!equal(expected, found)) error("Expected '" + expected + "' but found '" + found + "'");
  }

  public static void assertEq(Object expected, Object found, String message) {
    if (!equal(expected, found)) error(message);
  }
  ...
  public static void rethrow(String message, Throwable e) {
    ...
    while (cause.getCause() != null && cause.getCause() != e) cause = cause.getCause();
  ...
  /**
   * Searches the inheritance hierarchy for a field with the the given name and makes sure it is
   * settable via Field.setAccesible
   *
   * @param fieldName
   * @param clazz
   * @return
  ...
  /**
   * Gets all the fields from from all classes in the inheritance hierarchy EXCEPT for any class
   * who's packages starts with \"java\"
   *
   * @param clazz
   * @return
   */
      ...
      if (farr != null) {
        for (Field f : farr) {
          if (!found.contains(f.getName())) {
            ...
            System.out.println(
                "This super class property is being skipped because it is being hidden by a child class property with the same name...is this a design mistake? "
                    + f);
          }
        }
      }
   ...
   */
  public static void setField(String name, Object value, Object o)
  ...
  /**
   * Searches the inheritance hierarchy for the first method of the given name (ignores case). No
   * distinction is made for overloaded method names.
   *
   * @param clazz
   * @param name
   * @return
  ...
  public static List<Method> getMethods(Class clazz, String name) {
    ...
    do {
      for (Method m : clazz.getMethods()) {
        if (m.getName().equalsIgnoreCase(name)) methods.add(m);
      }
    } while (clazz != null && !Object.class.equals(clazz));
  ...
  /**
   * Tries to find a bean property getter then tries Field value, then defaults to the supplied
   * defaultVal
   *
   * @param name
   * @param object
  ...
  /**
   * Utility to call a close() method on supplied objects if it exists and completely ignore any
   * errors
   *
   * @param toClose
   */
  ...
  /**
   * Read all of the stream to a string and close the stream. Throws RuntimeException instead of
   * IOException
   *
   * @param in
   * @return
  ...
  public static File createTempFile(File file) throws IOException {
    if (file == null) return createTempFile("working.tmp");
    else return createTempFile(file.getName());
  }

  public static File createTempFile(String fileName) throws IOException {
    if (empty(fileName)) fileName = "working.tmp";
  ...
  /**
   * Attempts to locate the stream as a file, url, or classpath resource and then reads it all as a
   * string
   *
   * @param fileOrUrl
   * @return
  ...
  public static String replaceGroup(
      String regex, String source, int groupToReplace, String replacement) {
    return replaceGroup(regex, source, groupToReplace, 1, replacement);
  }

  public static String replaceGroup(
      String regex, String source, int groupToReplace, int groupOccurrence, String replacement) {
  ...
  public static String getShortCause(Throwable t, int lines) {
    t = getCause(t);
    // return System.getProperty("line.separator") + limitLines(clean(getStackTraceString(t)),
    // lines);
  ...
  /**
   * Pattern matches the string using ? to indicate any one single value and * to indicate any 0-n
   * multiple values
   */
  public static boolean wildcardMatch(String wildcard, String string) {
    if (wildcard.equals("*")) return true;

    if (empty(wildcard) || empty(string)) return false;

    if (!isWildcard(wildcard)) return wildcard.equals(string);
    else return regexMatch(wildcardToRegex(wildcard), string);
  }
  ...
  public static boolean regexMatch(String regex, String string) {
    if (empty(regex) || empty(string)) return false;
  ...
  public static boolean findSysEnvPropBool(String name, Object overrideValue) {
    Object obj = findSysEnvProp(name, overrideValue);
    if (obj != null) return "true".equalsIgnoreCase(obj.toString());
    return false;
  }
  ...
  public static Object findSysEnvProp(String name, Object overrideValue) {
    if (!Utils.empty(overrideValue)) return overrideValue;
    ...
    if (Utils.empty(value)) value = System.getenv(name);
