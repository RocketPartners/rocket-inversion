...
public abstract class Db<T extends Db> {
  ...
  /** Made to be overridden by subclasses or anonymous inner classes to do specific init */
  ...
  public Results<Row> select(Table table, List<Term> columnMappedTerms) throws Exception {
    throw new ApiException(
        ...
        "Unsupported Operation.  Implement " + getClass().getName() + ".select()");
  }
  ...
  public void delete(Table table, String entityKey) throws Exception {
    throw new ApiException(
        SC.SC_500_INTERNAL_SERVER_ERROR,
        "Unsupported Operation.  Implement " + getClass().getName() + ".delete()");
  }

  public String upsert(Table table, Map<String, Object> row) throws Exception {
    throw new ApiException(
        SC.SC_500_INTERNAL_SERVER_ERROR,
        "Unsupported Operation.  Implement " + getClass().getName() + ".upsert()");
  }
  ...
  public Object cast(String type, Object value) {
    try {
      ...
      switch (type) {
        ...
        case "N":
          if (value.toString().indexOf(".") < 0) return Long.parseLong(value.toString());
          else Double.parseDouble(value.toString());
  ...
  public Table getTable(String tableName) {
    for (Table t : tables) {
      if (t.getName().equalsIgnoreCase(tableName)) return t;
    }
    ...
    for (Table t : tables) {
      ...
      if (name.indexOf(" ") > -1) {
        ...
        if (name.equalsIgnoreCase(tableName)) return t;
      }
    }
  ...
  public T withTables(Table... tbls) {
    for (Table table : tbls) withTable(table);
  ...
  public T withCollectionPath(String collectionPath) {
    if (collectionPath != null && !collectionPath.endsWith("/")) collectionPath += "/";
  ...
  protected String beautifyCollectionName(String name) {
    ...
    if (!(name.endsWith("s") || name.endsWith("S"))) name = English.plural(name);
    ...
    for (int i = 0; i < name.length(); i++) {
      ...
      if (buff.length() == 0
          ...
          !(Character.isAlphabetic(next) //
              || next
                  == '$')) // OK $ is a valid initial character in a JS identifier but seriously why
                           // dude, just why?
      {
        next = 'x';
      }
      ...
      if (buff.length() == 0) next = Character.toLowerCase(next);
  ...
  protected String makeRelationshipUniqueName(Entity entity, Relationship rel) {
    ...
    if (type.equals(Relationship.REL_ONE_TO_MANY)) {
      ...
      if (idxColName.toUpperCase().equals(idxColName)) idxColName = idxColName.toLowerCase();
      ...
      if (!collectionName.equalsIgnoreCase(idxColName) //
        ...
        name =
            idxColName
                + Character.toUpperCase(relatedCollectionName.charAt(0))
                + relatedCollectionName.substring(1, relatedCollectionName.length());
  ...
  public Set<Term> mapToColumns(Collection collection, Term term) {
    ...
    if (term.isLeaf() && !term.isQuoted()) {
      ...
      if (attr != null) {
        ...
        if (term.getToken().startsWith("-")) columnName = "-" + columnName;
