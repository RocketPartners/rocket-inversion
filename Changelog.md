...
- LIFT-2099: Rework wildcard relevance scoring using the Elasticsearch `rescore` API
  ...
  - Fixes timeouts on large indices where the previous `_score`-sort approach forced every shard to score every filter-matched document
  - Removes silent sort demotion: callers with `sort=id` now actually get sorted by id
